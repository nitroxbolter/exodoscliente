...
local mouseGrabberSelectItem = nil
local learned_spells = {}
local selectedWigdet = nil
local lastAction = 0
local TYPE = {
  BLANK = 0,
  TEXT = 1,
...
function init()
  ...
  mouseGrabberSelectItem = g_ui.createWidget('UIWidget')
  mouseGrabberSelectItem:setVisible(false)
  mouseGrabberSelectItem:setFocusable(false)
  mouseGrabberSelectItem.onMouseRelease = onSelectItem

  ProtocolGame.registerExtendedOpcode(59, onReceivedSpellList)
end
...
function createActionBars()
  ...
  parent = modules.game_interface.getActionBar()
  index = 1
  layout = 'actionbar'

  actionBars[1] = g_ui.loadUI(layout, parent)
  actionBars[1]:setId("actionbar." .. 1)
  actionBars[1].n = 1

  actionBars[1]:fill('parent')

  parent:moveChildToIndex(actionBars[1], index)
end
...
function onSelectItem(self, mousePosition, mouseButton)
  ...
  local itemWidget = cachedSettings.widget.item
  local item = nil
  ...
  itemWidget:setItem(Item.create(item:getId(), item:getCount()))
  g_mouse.popCursor('target')
  self:ungrabMouse()
  return true
end
...
function setupActionBar(n)
  ...
  if actionbar.configureProfileButton then
    actionbar.configureProfileButton:setTooltip(tr('Switch selected profile. Current profile: %s',
      g_settings.getNumber('profile')))
  end
...
function setupButton(widget)
  ...
  widget.onMouseRelease = function(widget, mousePos, mouseButton)
    if mouseButton == MouseRightButton then
      ...
      menu:addOption(widget.spellId and tr('Edit Spell') or tr('Assign Spell'), function() getLearnedSpells(widget) end)
      menu:addOption(widget.item:getItemId() > 100 and tr('Edit Object') or tr('Assign Object'),
        function() assignItem(widget) end)
      menu:addOption(widget.text:getText():len() > 0 and tr('Edit Text') or tr('Assign Text'),
        function() assignText(widget) end)
  ...
  elseif widget.type == TYPE.TEXT then
    actionDesc = tr('Say') .. ': "' .. widget.text:getText() .. '"\n'
    actionDesc = actionDesc .. tr('Auto sent') .. ":  " .. (widget.autoSay and tr("Yes") or tr("No"))
  elseif widget.type == TYPE.SPELL then
    ...
    actionDesc = tr('Cast') .. " " .. spellData.name .. "\n"
    actionDesc = actionDesc .. tr('Formula') .. ":  " .. spellData.words .. paramText .. "\n"
    actionDesc = actionDesc .. tr('Cooldown') .. ":  " .. spellData.cd .. "s\n"
    actionDesc = actionDesc .. tr('Mana') .. ":  " .. spellData.mana
  elseif widget.type == TYPE.ITEM then
    actionDesc = tr(itemAction)
  end

  local hotkeyDesc = widget.hotkey and widget.hotkey:len() > 0 and widget.hotkey or "None"
  local tooltip = tr('Action Button') .. " " .. id
  tooltip = tooltip .. "\n\n\t" .. tr('Action') .. ":  " .. actionDesc
  tooltip = tooltip .. "\n" .. tr('Hotkeys') .. ":  " .. hotkeyDesc
...
function assignItem(widget)
  ...
  -- basics
  window:setText(tr('Assign Object to Action Button') .. " " .. widget:getId())
  window:setId("assignItemWindow")

  -- select item
  window.select.onClick = function()
    startChooseItem(widget)
  end
...
function startChooseItem(widget)
  if g_ui.isMouseGrabbed() then return end
  cachedSettings = { id = widget:getId(), data = settings[widget:getId()], widget = widget }
  mouseGrabberSelectItem:grabMouse()
  g_mouse.pushCursor('target')
end
...
function onReceivedSpellList(protocol, opcode, buffer)
  local spellIds = buffer:split(",")
  learned_spells = {}

  for _, spellId in ipairs(spellIds) do
    spellId = tonumber(spellId)
    local spell, profile, k = Spells.getSpellById(spellId)
    if spell then
      table.insert(learned_spells, tonumber(spellId))
    end
  end
  assignSpell(selectedWigdet)
end

function getLearnedSpells(widget)
  selectedWigdet = widget
...
function assignSpell(widget)
  ...
  window:setText(tr("Assign Spell to Action Button") .. " " .. widget:getId())
  ...
  for spellName, spellData in pairs(spells) do
    local has = isInTable(learned_spells, tonumber(spellData.id))
    ...
    if has then
      ...
      widget.source = spellData.icon
      widget.image:setImageSource(spellData.icon)
...
function assignHotkey(widget)
  ...
  local barDesc = tr("Action Bar: Action Button") .. " " .. widget:getId()
  window:setText(tr('Edit Hotkey for') .. ' "' .. barDesc)
  ...
  if not settings[widget:getId()] then
...
function setupAction(widget)
  ...
  if widget.type == TYPE.TEXT then
    widget.callback = function()
      if lastAction + 250 > g_clock.millis() then return end
